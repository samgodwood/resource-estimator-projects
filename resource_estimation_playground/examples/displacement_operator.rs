...
// Function to calculate the number of Rz gates, Rz gate depth, and number of qubits based on cutoff, max_qubits, and parallel_operations
fn displacement_params(cutoff: usize, max_qubits: usize, parallel_operations: usize) -> (usize, usize, usize) {
    let rz_gates = ((5.94 * (cutoff as f64).powf(1.16)).round() as usize) * parallel_operations;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for hilbert_cutoff in [10, 20, 50] {
        let (rz_gates, rz_depth, n_qubits) = displacement_params(hilbert_cutoff, 50, 1);
    ...
    let fixed_cutoff = 50;
    let (rz_gates, rz_depth, n_qubits) = displacement_params(fixed_cutoff, 50, 1);
