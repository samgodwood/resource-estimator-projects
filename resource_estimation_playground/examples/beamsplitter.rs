// Use build frontier for a bosonic beamsplitter using qubits.
...
// Function to calculate the number of Rz gates, Rz gate depth, and number of qubits based on cutoff and max_qubits
fn beamsplitter_params(cutoff: usize, max_qubits: usize, parallel_operations: usize) -> (usize, usize, usize) {
    ...
    if n_qubits > max_qubits {
        panic!(
            "number of qubits exceeds max for cutoff = {} and parallel_operations = {}",
            cutoff, parallel_operations
        );
    }
...
// Function to calculate fidelity based on photon loss rate with gate time = 10ns
fn fidelity(photon_loss: f64) -> f64 {
    1.1266* (-17.2832 * photon_loss).exp() - 0.1270
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for hilbert_cutoff in [5, 10, 20] {
        let (rz_gates, rz_depth, n_qubits) = beamsplitter_params(hilbert_cutoff, 50, 1);
    ...
    let fixed_cutoff = 30;
    let (rz_gates, rz_depth, n_qubits) = beamsplitter_params(fixed_cutoff, 50, 1);
    ...
    let mut file = File::create("./results/beamsplitter/beamsplitter.json")?;
    file.write_all(json_results.to_string().as_bytes())?;

    println!("Results written to ./results/beamsplitter/beamsplitter.json");
